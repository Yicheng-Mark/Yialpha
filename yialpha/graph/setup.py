...
class GraphSetup:
    ...
    def __init__(
        ...
        *,
        ...
        argumentation. The caller (``YiAlphaGraph``) resolves the
        ...
        * ``perf_tracker`` — when not None, every node handler is wrapped with
          :func:`yialpha.graph.perf_telemetry.wrap_node` to record per-node

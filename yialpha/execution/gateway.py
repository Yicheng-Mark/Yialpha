...
(https://github.com/vnpy/vnpy/blob/master/LICENSE). Adapted for YiAlpha:
...
* **Fully synchronous.** vnpy's ``BaseGateway`` takes an ``EventEngine`` and
  ...
  on the engine's background threads). YiAlpha is a single-process,
...
* **``send_order`` returns ``OrderData`` synchronously** instead of a
  ...
  order, get one result), which is the only execution shape YiAlpha needs.
* **Dropped** subscribe / quote / history / write_log (no tick stream, no
  two-sided quotes, YiAlpha has its own market-data layer, and logging uses
...
``yialpha.dataflows.binance.proxy_map()`` (the project-wide SOCKS5h pattern)
...
will wrap the existing ``yialpha.execution.browser_broker.BrowserBroker``

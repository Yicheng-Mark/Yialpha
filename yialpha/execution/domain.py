...
* **Pruned** to the subset needed for single-symbol batch order execution —
  tick / bar / quote / contract / history / log data are removed (YiAlpha has
  its own read-only market-data layer in ``yialpha.dataflows`` and never
...
class Product(Enum):
    """Product class — trimmed to the asset types YiAlpha trades."""
...
class Exchange(Enum):
    """Exchange — trimmed to YiAlpha' markets.
...
class BaseData:
    ...
    (vnpy's ``extra`` passthrough field is dropped — YiAlpha has no use for it.)
    """

"""Fama-French factor attribution for the YiAlpha backtest harness.
...
:mod:`yialpha.backtest.metrics`' naive ``alpha_vs_buyhold`` (a plain
...
def _default_cache_dir() -> Path:
    # Resolve through the config layer (data_cache_dir / YIALPHA_CACHE_DIR),
...
def _cached_or_download(path: Path, url: str, ttl_days: float = 1.0) -> bytes | None:
    ...
    try:
        ...
        if path.exists():
            ...
            from yialpha.dataflows.disk_cache import max_stale_days

...
from yialpha.dataflows import quality  # noqa: E402
from yialpha.default_config import DEFAULT_CONFIG  # noqa: E402
from yialpha.graph.analyst_execution import (  # noqa: E402
...
from yialpha.reporting import write_report_tree  # noqa: E402
...
app = typer.Typer(
    name="YiAlpha",
    help="YiAlpha CLI: Multi-Agent LLM Financial Analysis Framework",
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]Welcome to YiAlpha CLI[/bold green]",
            title="Welcome to YiAlpha",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections(asset_type_override: str = "auto"):
    ...
    # Step 3: Output language (skipped when set via YIALPHA_OUTPUT_LANGUAGE)
    if os.environ.get("YIALPHA_OUTPUT_LANGUAGE"):
    ...
    if asset_type == _AssetType.CRYPTO_PERP:
        from yialpha.dataflows.binance import warm_equity_perp_bases
    ...
    # supplied through YIALPHA_MAX_DEBATE_ROUNDS / _MAX_RISK_ROUNDS we keep
    # the run non-interactive and honor the env values (#977).
    depth_from_env = bool(os.environ.get("YIALPHA_MAX_DEBATE_ROUNDS")) and bool(
        os.environ.get("YIALPHA_MAX_RISK_ROUNDS")
    )
    if depth_from_env:
    ...
    # Step 6: LLM Provider (skipped when set via YIALPHA_LLM_PROVIDER).
    # The backend URL comes from YIALPHA_LLM_BACKEND_URL when set,
    # otherwise the provider's default endpoint — the same value the menu
    # would have picked.
    provider_from_env = bool(os.environ.get("YIALPHA_LLM_PROVIDER"))
    ...
    if os.environ.get("YIALPHA_QUICK_THINK_LLM") or os.environ.get("YIALPHA_DEEP_THINK_LLM"):
    ...
    # settable via its YIALPHA_* env var; when that var is set (or the
...
def _build_run_config(selections: dict, checkpoint: bool | None) -> dict:
    ...
    # (YIALPHA_MAX_DEBATE_ROUNDS / _MAX_RISK_ROUNDS) wins over the
    # interactive selection — leave the env-applied value in place (#977).
    if not os.environ.get("YIALPHA_MAX_DEBATE_ROUNDS"):
        config["max_debate_rounds"] = selections["research_depth"]
    if not os.environ.get("YIALPHA_MAX_RISK_ROUNDS"):
...
def run_analysis(checkpoint: bool | None = None, asset_type: str = "auto"):
    ...
    # explicit YIALPHA_DATA_VACUUM_POLICY wins over this default (someone who
    # sets it deliberately wants the typed failure even interactively).
    if "YIALPHA_DATA_VACUUM_POLICY" not in os.environ:
    ...
    from yialpha.batch.runner import serialized_run
    ...
    with serialized_run(
        ...
        # YiAlphaGraph._run_graph. langgraph executes node tasks inside
...
def _warn_config_drift() -> None:
    ...
    *noticed*: ``yialpha snapshot record`` pins a config, and this check
    ...
    snapshot. Point the user at ``yialpha snapshot diff`` for the detail.
    """
    try:
        from yialpha.config_snapshot import diff_against_last_snapshot
        ...
        if diffs:
            console.print(
                ...
                f"{'…' if len(diffs) > 8 else ''}). Run `yialpha snapshot diff` "
                f"for details, then `yialpha snapshot record` after applying "
...
def analyze(
    checkpoint: bool | None = typer.Option(
        ...
        "crashed run can resume). Omit to honor YIALPHA_CHECKPOINT_ENABLED.",
    ),
    ...
    if clear_checkpoints:
        from yialpha.graph.checkpointer import clear_all_checkpoints
...
def batch(
    ...
    workers: int | None = typer.Option(
        ...
        "when omitted, honor YIALPHA_BATCH_CONCURRENCY/BATCH_WORKERS.",
    ),
):
    ...
    env switch ``YIALPHA_BATCH_CONCURRENCY=true`` also enables the pool.
    """
...
def verify_history_cmd(
    ...
    """
    from yialpha.accuracy import verify_history as run_verify
...
def memory_resolve_cmd(
    ...
    from yialpha.dataflows.config import set_config as _set_config
...
def config_check():
    ...
    from yialpha.dataflows import tavily as _tavily
    from yialpha.dataflows.config import get_config as _get_config
    ...
    _SPLIT = "YIALPHA_TAVILY_BUDGET" + "_SPLIT"
    ...
    # -- Known risk items -----------------------------------------------------
    console.print("\n[dim]Risk items:[/dim]")
    _TIMEOUT = "YIALPHA_LLM_TIMEOUT" + "_S"
    ...
    from yialpha.agents.analysts.market_analyst import INDICATOR_NAMES
    ...
    if ready:
        ...
        console.print(
            ...
            f"set {key_env} or change YIALPHA_LLM_PROVIDER.[/bold red]"
        )
        raise typer.Exit(code=1)
...
# --------------------------------------------------------------------------- #
# Runtime wiring for yialpha/config_snapshot.py (the mechanism existed but had
...
def snapshot_record(
    ...
    from yialpha.config_snapshot import record_config_snapshot
    from yialpha.dataflows.config import get_config
...
def snapshot_list(
    ...
    from yialpha.config_snapshot import list_snapshots
    from yialpha.dataflows.config import get_config

    snapshots = list_snapshots(get_config())
    if not snapshots:
        console.print("[yellow]No config snapshots recorded yet.[/yellow]")
        console.print('[dim]Record one with: yialpha snapshot record --reason "..."[/dim]')
        return
    table = Table(box=box.SIMPLE)
...
def ic_cycle(
    ...
    **Never edits the live config — review, then apply by hand.**
    ...
    if common is not None and any(v["prune"] for v in result["verdict"].values()):
        ...
        console.print(
            f"[dim]yialpha snapshot record --reason 'IC prune <date>' "

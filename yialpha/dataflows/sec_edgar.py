...
:func:`yialpha.dataflows.utils.is_filing_public` only for the rare fact with no
...
``YIALPHA_SEC_USER_AGENT`` (e.g. ``"YiAlpha research you@example.com"``) to
...
def _user_agent() -> str:
    return os.environ.get("YIALPHA_SEC_USER_AGENT") or "YiAlpha research (sec-edgar vendor)"

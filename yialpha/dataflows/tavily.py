...
tunable via ``YIALPHA_TAVILY_BUDGET_SPLIT``.
...
#: Default network timeout (seconds); override with YIALPHA_TAVILY_TIMEOUT_S.
DEFAULT_TIMEOUT_S = 20
#: Default per-run search-call cap (TOTAL across scopes); override with
#: YIALPHA_TAVILY_MAX_CALLS_PER_RUN.
...
#: YIALPHA_TAVILY_BUDGET_SPLIT replaces it wholesale (scopes omitted get 0).
...
_calls_var: ContextVar[dict[str, int] | None] = ContextVar(
    "yialpha_tavily_calls", default=None
)
...
_key_cursor_var: ContextVar[dict[str, int] | None] = ContextVar(
    "yialpha_tavily_key_cursor", default=None
)
_dead_keys_var: ContextVar[set[int] | None] = ContextVar(
    "yialpha_tavily_dead_keys", default=None
)
...
def parse_budget_split(raw: str) -> dict[str, int] | None:
    ...
    Public so ``yialpha config-check`` validates the same parse the runtime
    uses.
    """
    split: dict[str, int] = {}
    try:
        ...
        logger.warning(
            "Ignoring malformed YIALPHA_TAVILY_BUDGET_SPLIT=%r; expected "
...
def _budget_split(total: int) -> dict[str, int]:
    ...
    raw = os.getenv("YIALPHA_TAVILY_BUDGET_SPLIT")
    if raw is not None:
...
def _max_calls_per_run() -> int:
    raw = os.getenv("YIALPHA_TAVILY_MAX_CALLS_PER_RUN")
    if raw is None:
    ...
    try:
        ...
        logger.warning(
            "Ignoring non-numeric YIALPHA_TAVILY_MAX_CALLS_PER_RUN=%r", raw,
        )
        return DEFAULT_MAX_CALLS_PER_RUN


def _timeout_s() -> float:
    raw = os.getenv("YIALPHA_TAVILY_TIMEOUT_S")
    if raw is None:
        return float(DEFAULT_TIMEOUT_S)
    try:
        return max(1.0, float(raw))
    except ValueError:
        logger.warning("Ignoring non-numeric YIALPHA_TAVILY_TIMEOUT_S=%r", raw)
        return float(DEFAULT_TIMEOUT_S)
...
def get_web_search(
    ...
    if scope_cap is None:
        return _sentinel(
            ...
            f"YIALPHA_TAVILY_BUDGET_SPLIT (allocated scopes: "
    ...
    if scope_used >= scope_cap:
        return _sentinel(
            ...
            "YIALPHA_TAVILY_BUDGET_SPLIT or YIALPHA_TAVILY_MAX_CALLS_PER_RUN "
    ...
    if total_used >= total_cap:
        return _sentinel(
            ...
            "YIALPHA_TAVILY_MAX_CALLS_PER_RUN if more angles are needed)."
        )
    _charge(scope)

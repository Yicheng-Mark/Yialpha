...
``YIALPHA_`` prefix; only ever in ``.env`` / gitignore). A missing token raises
...
``YIALPHA_A_SHARE_NATIVE`` is on AND ``is_a_stock(ticker)`` holds.
"""
...
def _to_tushare_code(ticker: str) -> str:
    """Map a YiAlpha A-share ticker to Tushare's ``600519.SH`` / ``000001.SZ``.

    Note Tushare uses ``.SH`` for Shanghai where YiAlpha/yfinance use ``.SS``;

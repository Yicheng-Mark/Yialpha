...
Like the BaoStock vendor this is a fresh, thin, synchronous YiAlpha module
...
only advertised to the news analyst when ``YIALPHA_A_SHARE_NATIVE`` is on
...
class _direct_connect:
    ...
    :mod:`yialpha.dataflows.timeout_shim`).
    """
...
def _to_akshare_code(ticker: str) -> str:
    """Map a YiAlpha A-share ticker to AKShare's bare 6-digit code.
...
def _market_for(ticker: str) -> str:
    """Derive the AKShare market prefix ('sh'/'sz') from a YiAlpha A-share ticker.

    ``stock_individual_fund_flow`` takes a bare 6-digit code **and** a market
    prefix (secid = ``{sh:1, sz:0}.{code}``); the suffix YiAlpha carries
...
def _baostock_industry(ticker: str) -> str | None:
    ...
    infrastructure is reused from :mod:`yialpha.dataflows.baostock_vendor`.

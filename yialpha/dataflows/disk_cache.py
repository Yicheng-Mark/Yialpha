...
layer to read ``YIALPHA_CACHE_DIR`` directly). This module is the single
...
* on a fetch failure a **stale** cache is served with a WARNING that includes
  the cache age, but only up to ``YIALPHA_DATA_CACHE_MAX_STALE_DAYS`` (default
...
``~/.yialpha/cache`` fallback in exactly one place).
...
_DEFAULT_CACHE_BASE = os.path.join(os.path.expanduser("~"), ".yialpha", "cache")
...
def max_stale_days() -> float:
    ...
    ``YIALPHA_DATA_CACHE_MAX_STALE_DAYS`` (default 30). ``0`` is valid and

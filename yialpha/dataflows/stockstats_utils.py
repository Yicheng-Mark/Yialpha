...
# YIALPHA_HTTP_TIMEOUT_S (seconds), default 30s ON:
...
# hang defeats even that — set YIALPHA_HTTP_TIMEOUT_S=0 to disable explicitly.
_HTTP_TIMEOUT_ENV = os.environ.get("YIALPHA_HTTP_TIMEOUT_S")
...
if _HTTP_TIMEOUT_ENV is not None and _HTTP_TIMEOUT_ENV != "":
    try:
        ...
        if parsed > 0:
            ...
            logger.warning(
                "YIALPHA_HTTP_TIMEOUT_S=%r is not positive; using the default "
        ...
        logger.warning(
            "YIALPHA_HTTP_TIMEOUT_S=%r is not a number; using the default "
            "%.0fs instead", _HTTP_TIMEOUT_ENV, _HTTP_TIMEOUT_DEFAULT,
        )
...
def filter_financials_by_date(data: pd.DataFrame, curr_date: str | None) -> pd.DataFrame:
    ...
    (see :func:`yialpha.dataflows.utils.is_filing_public`). We keep a column

...
The chain (first wins):
  ...
  2. ``YIALPHA_LLM_TIMEOUT_S`` env var     (operator-wide override)
...
A non-numeric ``YIALPHA_LLM_TIMEOUT_S`` (e.g. ``120s`` with a unit suffix) is
...
# has not set ``YIALPHA_LLM_TIMEOUT_S``. Real cloud calls land in <10s; 120s
...
def resolve_timeout(
    ...
    *,
    ...
    raw = os.environ.get("YIALPHA_LLM_TIMEOUT_S")
    if raw:
        ...
        # local/cloud logic.
        logger.warning(
            "YIALPHA_LLM_TIMEOUT_S=%r is not a positive finite number; ignored "
    ...
    with _timeout_warn_lock:
        if not _timeout_warned:
            ...
            logger.info(
                "YIALPHA_LLM_TIMEOUT_S unset: cloud LLM calls use the built-in "
                "%ss read-timeout default. Set YIALPHA_LLM_TIMEOUT_S to "

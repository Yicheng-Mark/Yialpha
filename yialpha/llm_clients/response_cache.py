...
Default-OFF dev/iteration accelerator. When ``YIALPHA_LLM_CACHE`` is set,
``YiAlphaGraph.__init__`` installs one ``DiskLLMCache`` via langchain's global
...
``yialpha.backtest.cache.DecisionCache``, which already makes whole-backtest
re-runs free.
...
per-call cache collapses that variability to a single realization, so:
    ...
    **Do NOT enable this cache while running the analyst A/B gate or a
    ...
    set ``YIALPHA_LLM_CACHE=true`` for those workflows.

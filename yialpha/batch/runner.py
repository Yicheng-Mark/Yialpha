...
Why this is safe (see plan yialpha-prancy-acorn.md):
  * A pool of K long-lived YiAlphaGraph instances, one per worker, never shared
  ...
  * Shared BACKING files (memory log, OHLCV cache) are serialized by their own
    file locks (yialpha.batch.locks) — installed unconditionally, harmless when
    uncontended.
...
def prepare_batch_run(
    ...
    ``yialpha batch`` command and ``scripts/run_batch.py`` agree on: ticker
...
class BatchRunner:
    ...
    selected_analysts : tuple, optional
        Passed through to every YiAlphaGraph (default: all four analysts).
    ...
    def __init__(
        ...
        *,
        ...
        graph_factory: Callable[[dict], YiAlphaGraph] | None = None,
        progress: bool = True,
    ):
        self.config = config
        ...
        # logger, so records from ``yialpha.*`` child loggers (where all of
    ...
    def _default_graph_factory(self, config: dict) -> YiAlphaGraph:
        return YiAlphaGraph(self.selected_analysts, debug=False, config=config)
    ...
    def _assert_uniform(self, graph_config: dict) -> None:
        if _config_signature(graph_config) != self._signature:
            raise ValueError(
                ...
                "LLM/vendor keys. YiAlphaGraph.__init__ mutates a module-global "

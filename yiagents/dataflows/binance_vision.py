...
Design constraints (mirrors :mod:`yiagents.dataflows.binance`):
  - side-effect-free import; proxies read at call time via ``proxy_map()``;
  - raises the typed errors from :mod:`yiagents.dataflows.errors` so the
  ...
  - downloaded zips are cached under ``<data_cache_dir>/binance_vision/``
    through the shared :mod:`yiagents.dataflows.disk_cache` contract —

...
:mod:`yiagents.dataflows.valuation_methods`). But analysts sometimes need to
...
For these, :class:`~yiagents.agents.utils.pot_integration.PotAnalyzer` asks the
LLM to write a short Python snippet, runs it in the restricted sandbox
(:mod:`yiagents.agents.utils.pot_executor`), and returns the computed answer.
...
``valuation_tools`` config flag (``YIAGENTS_VALUATION_TOOLS``). When off, the
...
from yiagents.agents.utils.pot_integration import PotAnalyzer

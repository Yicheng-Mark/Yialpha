...
``<results_dir>/<TICKER>/YiAgentsStrategy_logs/full_states_log_<date>.json``
...
The heavy lifting runs in the CLI (``yiagents verify-history``) which writes
...
def fetch_returns_yf(
    ...
    Same semantics as ``YiAgentsGraph._fetch_returns`` (which delegates here):
